...
void UART_SCBCLK_SetFractionalDividerRegister(uint16 clkDivider, uint8 clkFractional)
{
    ...
    uint32 maskVal = UART_SCBCLK_DIV_REG & 
                    (uint32)(~(UART_SCBCLK__DIVIDER_MASK | UART_SCBCLK__FRAC_MASK)); 
	/* combine mask and new divider val into 32-bit value */
    uint32 regVal = clkDivider | (((uint32)clkFractional << 16) & UART_SCBCLK__FRAC_MASK) | maskVal;
#else
    /* get all but integer divider bits */
    uint32 maskVal = UART_SCBCLK_DIV_REG & (uint32)(~(uint32)UART_SCBCLK__DIVIDER_MASK);
    /* combine mask and new divider val into 32-bit value */
...
{
    return (uint16)UART_SCBCLK_DIV_REG;
}
...
uint8 UART_SCBCLK_GetFractionalDividerRegister(void)
{
    ...
    uint32 maskVal = UART_SCBCLK_DIV_REG & UART_SCBCLK__FRAC_MASK;
    return (maskVal >> 16);
#else
    return 0u;
#endif /* UART_SCBCLK__FRAC_MASK */
